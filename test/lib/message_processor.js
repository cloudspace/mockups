...
exports.process = testCase({
	...
	"project_find: sends a project_prompt_password message when there is a project password ": function(test) {
		var that = this;
		Project.create(function(project) {
			...
			project.update({ password: "Surely you can't be serious." },function(update_statement){
				...
				setTimeout(function() {
					test.equal(that.client.sent.project_prompt_password, true);
					test.done();
				}, 500);
				});
		});
