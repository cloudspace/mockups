	...
	page_update: function(client, data) {
		...
		Page.find_by_id_and_project_id(data.page.id, client.user.project_id, function(page) {
			page.update(data.page, function(page) {
				...
				client.user.broadcast_to_project({ page_update: data}, true);
		...
		Page.find_by_id_and_project_id(data.page_id, client.user.project_id, function(page) {
			page.delete(function(page){
				if (page.error) {
					...
					client.user.broadcast_to_project(msg, true);
					client.user.broadcast_to_project({ page_delete: { page: page } }, true);
				}
			});
		});
	},
	...
	canvas_object_create: function(client, data) {
		...
		Page.find_by_id_and_project_id(data.page.id, client.user.project_id, function(page) {
			...
			CanvasObject.create(page, data, function(canvas_object) {
				client.user.broadcast_to_project({ message: client.user.handle() + ' added a canvas object.'}, true);
				client.user.broadcast_to_project({ canvas_object_create: { canvas_object: canvas_object } }, false);
				client.send({ canvas_object_create: { canvas_object: canvas_object, focus: true } });
			});
		});
	},
