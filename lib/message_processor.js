...
var MessageProcessor = {
  ...
  project_find: function (client, data) {
	...
	canvas_object_update: function (client, data) {
		...
		Project.findById(client.user.project_id, function (err, project) {
			var canvas_object = project.pages.id(data.page._id).canvas_objects.id(data.canvas_object._id);
			canvas_object.set(data.canvas_object);
			project.save(function (err) {
				if (err) {
					client.send({ error: err });
					return;
	...
	canvas_object_delete: function (client, data) {
		...
		Project.findById(client.user.project_id, function (err, project) {
			var canvas_object = project.pages.id(data.page._id).canvas_objects.id(data.canvas_object._id).remove();
			project.save(function (err) {
				if (err) {
					client.send({ error: err });
					return;
				}
				client.user.broadcast_to_project({ message: client.user.handle() + ' deleted element: ' + canvas_object.name + '.'}, true);
